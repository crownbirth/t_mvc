...
class Payment extends CI_Controller {
    ...
    */    
    ...
    public function setexception($scheduleid){
        ...
        $level = json_decode($schedule['eliglevel']);
        $eligible = $schedule['eligible'];
        $eligtype = jason_decode($schedule['eligtype']);
        
        if($eligible = 'college'){
            $this->util_model->ge;
            //$this->util_model->get_data('college', array('colid', 'colname'), )
        }
    ...
    */
    public function myschedule(){
        ...
        $schedule_param = array(
                                ...
                                "usertype" => $this->main->item('user_id'),
                                
                            );
        $my_pay_hist =  $this->pay_mdl->sch1( $pay_hist_param );
    ...
    */  
    ...
    public function sets($what){
        ...
        switch ($what) {
            ...
            case 'schedule':
                    if($this->main->has_perm('payment', 'payment.setup.create.payschedule')){
    ...
    public function process_payment(){
        ...
        if($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $parameter = array(
                                ...
                                'can_name' => $form_fields['can_name'],
                                'paid_url' => 'payment/pay/paid',
                                'cancle_url' => 'payment/pay/cancle',
                                'decline_url' => 'payment/pay/decline'
                        );
